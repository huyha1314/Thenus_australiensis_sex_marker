...
pixi run seqkit seq -m 2000 scaffolded_genome3/assembly.fa > scaffolded_genome3/assembly.2kb.masked.fa

pixi run hisat2-build \
  scaffolded_genome3/assembly.2kb.masked.fa \
  lobster_index

# 2. Align RNA-seq reads
pixi run hisat2 -p 48 -x lobster_index \
  -1  RNA/nf/results/cat/pooled_reads_1.merged.fastq.gz \
  -2  RNA/nf/results/cat/pooled_reads_2.merged.fastq.gz | \
pixi run samtools sort -@10 -m 3G -o rna_alignments.bam
pixi run samtools index rna_alignments.bam
...
export BAMTOOLS_PATH=/home/huyha/micromamba/envs/augustus/bin
 ...
 pixi run braker.pl \
  ...
  --gff3 
pixi run Launch_PASA_pipeline.pl \
