...
fastp \
        ...
        --json result/multiqc/${sample}.report.json 
```
...
seqkit seq -m 2000 scaffolded_genome3/assembly.fa > 2kb.Thenus_australiensis.genome.fa

hisat2-build \
  2kb.Thenus_australiensis.genome.fa \
  lobster_index
...
export BAMTOOLS_PATH=micromamba/envs/augustus/bin
 ...
 braker.pl \
  --genome= script_new/2kb.Thenus_australiensis.genome.fa  \
...
singularity exec \
  ...
  -C -R \
  -g 2kb.Thenus_australiensis.genome.fa \
...
EVidenceModeler \
  --sample_id lobster4 \
  --genome 2kb.Thenus_australiensis.genome.fa \
...
```bash
seqkit split2 -p 20 -O chunks_dir Thenus_australiensis.genome.fa
...
cmsearch --cpu 24  --rfam --tblout genome.rfam.tbl \
  rna/db/Rfam.cm \
  Thenus_australiensis.genome.fasta > genome.fa.rfam
```
...
seqkit split2 Thenus_australiensis.genome.fa -p 20 -O genome_parts
...
### 6.4. Annotation of Repeat Regions
```bash
mkdir -p SSR; cd SSR

perl MISA/25082020/outpAs.misa.pl Thenus_australiensis.genome.fa &>/dev/null
...
### 8.2 Step two: Sex-specific k-mers and reads extraction.
```bash
#WGS
...
### 8.3 Step three: De novo assembly of sex-specific reads.
```bash
...
### 8.4 Step four: Check against the candidates.
```bash
