...
patch_small_gaps <- function(blocks_df, max_gap = 50) {
  if (is.null(blocks_df) || nrow(blocks_df) <= 1) return(blocks_df)
  patched <- blocks_df[1, , drop=FALSE]
  for (i in 2:nrow(blocks_df)) {
    if ((blocks_df$Start[i] - patched$End[nrow(patched)]) <= max_gap) {
      patched$End[nrow(patched)] <- blocks_df$End[i]
    } else {
      patched <- rbind(patched, blocks_df[i, ])
    }
  }
  return(patched)
}

long_blocks <- patch_small_gaps(get_blocks(align_df$Long_Cov, 2.0, 2.6))
...
long_domain_blocks <- patch_small_gaps(get_blocks(align_df$Long_Domain, 2.1, 2.5))
