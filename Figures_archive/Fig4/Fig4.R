...
neoy_exons <- data.frame(
  start = c(4841, 10528, 13757, 17006, 23133, 35621, 40650, 42944, 53088, 59727, 67104, 75905, 84950),
  end   = c(4890, 10673, 13882, 17129, 23300, 35718, 40855, 43074, 53225, 59935, 67226, 75996, 86286),
  type = "Neo-Y Exon"
)
host_exons <- data.frame(
  start = c(1454, 3008, 3648, 6204, 8383, 9181, 10078, 12184, 12803, 13969, 15178, 15831, 17138, 17896, 19480, 20685, 22054, 22998, 23921, 25036, 30593, 31582, 31986, 33657, 37536),
  end   = c(1746, 3204, 3772, 6318, 8535, 9276, 10278, 12314, 12964, 14091, 15395, 16036, 17408, 18124, 19601, 20813, 22160, 23207, 24290, 29988, 30680, 31734, 32132, 34048, 37985),
  type = "Host Exon"
)
transposons <- data.frame(x = c(58813, (58813+61548)/2, 61548, (58813+61548)/2, 104720, (104720+106467)/2, 106467, (104720+106467)/2),
                          y = c(0.7, 0.85, 0.7, 0.55, 0.7, 0.85, 0.7, 0.55), group = c(rep("Tc1",4), rep("LINE",4)))
...
p_arch <- ggplot() +
  ...
  # Neo-Y track (Top, y = 0.7)
  geom_segment(aes(x = 0, xend = 122615, y = 0.7, yend = 0.7, color = "Intron"), linewidth = 1) +
  annotate("rect", xmin = 4841, xmax = 86286, ymin = 0.65, ymax = 0.75, fill = neo_y_green, alpha = 0.2) +
  geom_rect(data = neoy_exons, aes(xmin = start, xmax = end, ymin = 0.6, ymax = 0.8, fill = "Neo-Y Exon")) +
  geom_polygon(data = transposons, aes(x = x, y = y, group = group, fill = "Transposon")) +
  
  # Host track (Bottom, y = 0.3)
  geom_segment(aes(x = 0, xend = 37985, y = 0.3, yend = 0.3, color = "Intron"), linewidth = 1) +
  annotate("rect", xmin = 1454, xmax = 37985, ymin = 0.25, ymax = 0.35, fill = host_navy, alpha = 0.2) +
  geom_rect(data = host_exons, aes(xmin = start, xmax = end, ymin = 0.2, ymax = 0.4, fill = "Host Exon")) +
  ...
  scale_fill_manual(name = "Feature Type:", 
                    values = c("Host Exon" = host_navy, "Neo-Y Exon" = neo_y_green, "Transposon" = transposon_purple)) +
