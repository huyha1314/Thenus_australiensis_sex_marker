...
option_list = list(
  make_option(c("-o", "--out"), type="character", default="Fig3S_Final", 
              help="Prefix for output files [default= %default]", metavar="character")
)
opt_parser = OptionParser(option_list=option_list)
opt = parse_args(opt_parser)

data <- read.csv("lobster_y_chromosome_candidates_separated.csv")
...
ggsave(paste0(opt$out, ".pdf"), plot=p1, width=14, height=7, device=cairo_pdf)
ggsave(paste0(opt$out, ".png"), plot=p1, width=14, height=7, dpi=300, bg="white")

cat("Successfully created", paste0(opt$out, ".pdf"), "and", paste0(opt$out, ".png"), "\n")
